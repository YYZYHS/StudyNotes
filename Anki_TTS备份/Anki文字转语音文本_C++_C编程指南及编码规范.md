...
在程序中，有时会遇到 if， else， return 的组合情况。为了避免不良风格，建议将简单的条件判断返回值的写法改为标准的 if， else 结构，或者进一步简化/为使用三元运算符的形式，以使代码更加清晰和简练。
...
## 指南及规范0101——类的构造函数、析构函数与赋值函数概述
在C++编程中，每个类可以有多个构造函数，包括一个拷贝构造函数和其他普通构造函数，但只有一个析构函数和一个赋值函数。如果开发者没有为类明确提供这些函数，C++编译器会自动为类生成四个默认的函数：一个无参数的默认构造函数用于创建新对象，一个拷贝构造函数用于根据另一个同类型的对象初始化新对象，一个析构函数用于对象销毁时执行清理工作，以及一个赋值函数用于定义对象之间如何通过等号进行赋值。这些自动生成的函数确保对象的基本操作如创建、复制、销毁和赋值可以顺利进行，即使开发者没有显式定义这些操作。

## 指南及规范0102——为什么程序员需要编写函数而不完全依赖自动生成？
程序员需要编写自定义函数而不完全依赖自动生成的函数，主要是为了更好地控制对象的生命周期和管理内存。虽然C++编译器可以自动生成构造函数、析构函数、拷贝构造函数和赋值操作符，这些自动生成的函数只提供基本的操作，如位拷贝，但不允许自定义对象的初始化和清理。这可能导致资源管理问题，尤其是当类中包含如指针这类需要特别管理的资源时。例如，如果类中有指针变量，缺省的拷贝构造函数和赋值操作符会进行位拷贝，这可能会导致两个对象指向同一内存地址，进而可能在对象销毁时重复释放同一内存，造成错误。因此，程序员通过编写自定义函数可以确保资源被正确管理，防止内存泄露和其他安全问题，从而保证程序的稳定性和安全性。

## 指南及规范0103——位拷贝（Bitwise Copy）V.S. 值拷贝（Deep Copy）
位拷贝和值拷贝这两种对象拷贝方式有各自的特点和适用场景。位拷贝，或者说浅拷贝，是一种直接逐位复制对象所有内存数据的方法，包括对象中的指针变量的内存地址。这种方式虽然快速直接，但如果对象中含有指针或动态分配的资源，则可能导致多个对象共享同一块内存。这在释放内存时可能会引起重复释放的问题或操作悬空指针，从而导致程序崩溃。相对的，值拷贝，也就是深拷贝，不仅复制对象的基础数据，还会为对象中的指针成员或动态分配的资源分配新的内存，并复制内容。这样，每个对象都拥有独立的内存空间，即使对象中包含指针，拷贝后的对象也不会与原对象共享内存，从而避免了位拷贝可能引起的内存管理问题，并确保每个对象独立管理其资源。例如，如果一个对象有一个指向动态分配内存的指针成员，在位拷贝的情况下，新对象和原对象的指针都会指向同一块内存，这可能导致如果释放其中一个对象的内存，另一个对象的指针会变成悬空指针。而在值拷贝的情况下，新对象将拥有一份独立的内存拷贝，即使释放其中一个对象的内存，另一个对象的指针仍然有效。因此，位拷贝适合用于简单的对象，而值拷贝则更适合处理包含指针或动态内存分配的复杂对象。

## 指南及规范0104 构造函数与析构函数的起源
构造函数与析构函数的作用是自动执行对象的初始化和清除，增强程序的安全性。它们的自动执行机制是，构造函数在对象创建时自动调用，析构函数在对象消亡时自动调用。命名规则规定，构造函数与析构函数必须与类名相同，析构函数在前面加上‘波浪号’以示区分。它们没有返回值类型，这区别于返回 void 的函数。设计它们的初衷是为了防止因变量未正确初始化或清除而引发的程序错误，提高代码的可靠性。

## 指南及规范0105 构造函数的初始化表
构造函数的初始化列表用于在进入构造函数体之前初始化成员变量，位置位于构造函数参数列表之后、函数体之前，语法形式维: 成员变量(参数)。初始化列表中的初始化在构造函数体执行之前完成。在继承关系中，派生类的构造函数必须在初始化列表中显式调用基类的构造函数。对于const类型的成员变量，只能在初始化列表中初始化，不能在构造函数体内赋值。成员对象的初始化方面，非内部数据类型应在初始化列表中初始化，避免默认构造和赋值带来的效率损失；内部数据类型可在初始化列表中初始化，也可在构造函数体内赋值，效率差别不大。使用初始化列表可以避免不必要的默认构造和赋值操作，提升程序效率，尤其对于自定义类型的成员对象，初始化列表方式更高效。对于简单的内部数据类型，构造函数体内赋值可能更直观，因此在选择初始化方式时，应在效率和代码可读性之间取得平衡。最佳实践是始终在初始化列表中初始化成员对象，特别是自定义类型的成员，并在继承结构中确保在派生类的初始化列表中正确调用基类构造函数。